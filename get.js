export function addElement(stuff) {
    ...
    for(const data of stuff) {
        ...
        clone.querySelector('.precio').innerHTML = `Precio: $${data.precio} `;
        clone.querySelector('.cantidad').innerHTML = `Cantidad: ${data.cantidad} `;
        clone.classList.add('element-clone'); // Add class for easier removal

        const editButton = document.createElement('button');
        editButton.innerHTML = 'editar';
        editButton.classList.add('edit-btn', 'bg-blue-500', 'text-white', 'p-2', 'm-2');
        editButton.onclick = function() {
            console.log(`Editar producto ${data.id}`);
            Editar(data.id)
        };
        ...
        deleteButton.onclick = function() {
            console.log(`Eliminar producto ${data.id}`);
            Eliminar(data.id);
        };
